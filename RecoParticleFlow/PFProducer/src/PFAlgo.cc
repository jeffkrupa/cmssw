...
void PFAlgo::reconstructParticlesNew( const reco::PFBlockHandle& blockHandle , TClonesArray *array){//,const edm::Event &iEvent,const edm::EventSetup &iSetup,const HcalDDDRecConstants *iRecNumber) {

  blockHandle_ = blockHandle;
  reconstructParticlesNew( *blockHandle_ , array);//, iEvent, iSetup, iRecNumber);
}
...
void PFAlgo::reconstructParticlesNew( const reco::PFBlockCollection& blocks, TClonesArray *array){//const edm::Event &iEvent,const edm::EventSetup &iSetup,const HcalDDDRecConstants *iRecNumber) {
  ...
  for( IBR io = otherBlockRefs.begin(); io!=otherBlockRefs.end(); ++io) {
    if ( debug_ ) std::cout << "Block number " << nblcks++ << std::endl; 
    processBlockNew( *io, hcalBlockRefs, ecalBlockRefs , rArray);
  }
  ...
  for( IBR ih = hcalBlockRefs.begin(); ih!=hcalBlockRefs.end(); ++ih) {
    if ( debug_ ) std::cout << "HCAL block number " << hblcks++ << std::endl;
    processBlockNew( *ih, empty, empty , rArray);
  }
  ...
  for( IBR ie = ecalBlockRefs.begin(); ie!=ecalBlockRefs.end(); ++ie) {
    if ( debug_ ) std::cout << "ECAL block number " << eblcks++ << std::endl;
    processBlockNew( *ie, empty, empty , rArray);
  }
...
void PFAlgo::processBlockNew( const reco::PFBlockRef& blockref,
                           std::list<reco::PFBlockRef>& hcalBlockRefs, 
                           std::list<reco::PFBlockRef>& ecalBlockRefs,
			   TClonesArray &rArray ) { 
  ...
  for(unsigned iEle=0; iEle<elements.size(); iEle++) {
	...
	*/ 
	...
	//     nSigmaECAL_*neutralHadronEnergyResolution(trackMomentum,clusterRef->positionREP().Eta())  ) { 
	  ...
	  unsigned tmpe = reconstructClusterNew( *clusterRef, ecalEnergy , rArray); 
	  (*pfCandidates_)[tmpe].setEcalEnergy( clusterRef->energy(), ecalEnergy );
	  (*pfCandidates_)[tmpe].setHcalEnergy( 0., 0. );
	  (*pfCandidates_)[tmpe].setHoEnergy( 0., 0. );
	  (*pfCandidates_)[tmpe].setPs1Energy( ps1Ene[0] );
	...
	*/
	...
	if ( neutralEnergy > std::max(0.5,nSigmaECAL_*resol) ) {
	  neutralEnergy /= slopeEcal;
	  unsigned tmpj = reconstructClusterNew( *pivotalRef, neutralEnergy , rArray); 
	  (*pfCandidates_)[tmpj].setEcalEnergy( pivotalRef->energy(), neutralEnergy );
	  (*pfCandidates_)[tmpj].setHcalEnergy( 0., 0. );
	  (*pfCandidates_)[tmpj].setHoEnergy( 0., 0. );
	  (*pfCandidates_)[tmpj].setPs1Energy( 0. );
  ...
  if( !(hfEmIs.empty() &&  hfHadIs.empty() ) ) {
	...
	tmpi = reconstructClusterNew( *clusterRef, energyHF , rArray);     
	(*pfCandidates_)[tmpi].setEcalEnergy( uncalibratedenergyHF, energyHF );
	(*pfCandidates_)[tmpi].setHcalEnergy( 0., 0.);
	(*pfCandidates_)[tmpi].setHoEnergy( 0., 0.);
	(*pfCandidates_)[tmpi].setPs1Energy( 0. );
...
// 		c0->layer()== PFLayer::HF_HAD );
	...
	energyHfHad = thepfEnergyCalibrationHF_->energyEmHad(0.0,
      ...
      unsigned tmpi = reconstructClusterNew( *chad, energyHfEm+energyHfHad , rArray);     
      (*pfCandidates_)[tmpi].setEcalEnergy( uncalibratedenergyHFEm, energyHfEm );
      (*pfCandidates_)[tmpi].setHcalEnergy( uncalibratedenergyHFHad, energyHfHad);
      (*pfCandidates_)[tmpi].setHoEnergy( 0., 0.);
      (*pfCandidates_)[tmpi].setPs1Energy( 0. );
...
//       cerr<<"not ready for navigation in the HF!"<<endl;
  ...
  for(unsigned i=0; i<hcalIs.size(); i++) {
	...
	if (useHO_) {
	  ...
	  (*pfCandidates_)[tmpi].setHcalEnergy(totalHcal, muonHcal);
        ...
        setHcalDepthInfoNew((*pfCandidates_)[tmpi], *hclusterref, rArray);
    ...
    */
	...
	bool useDirection = true;
	unsigned tmpi = reconstructClusterNew( *pivotalClusterRef[iPivot], 
					    particleEnergy[iPivot],
					    rArray,                                            
					    useDirection,
	                                    particleDirection[iPivot].X(),
    ...
    */
    ...
    for ( IS is = ecalSatellites.begin(); is != ecalSatellites.end(); ++is ) { 
      ...
      // Create a photon
      unsigned tmpi = reconstructClusterNew( *eclusterref, sqrt(is->second.second.Mag2()) , rArray); 
      (*pfCandidates_)[tmpi].setEcalEnergy( eclusterref->energy(),sqrt(is->second.second.Mag2()) );
      (*pfCandidates_)[tmpi].setHcalEnergy( 0., 0. );
      (*pfCandidates_)[tmpi].setHoEnergy( 0., 0. );
      (*pfCandidates_)[tmpi].setPs1Energy( associatedPSs[iEcal].first );
  ...
  for(unsigned ihcluster=0; ihcluster<hcalIs.size(); ihcluster++) {
    ...
    unsigned tmpi = reconstructClusterNew( *hclusterRef, 
                                        calibEcal+calibHcal, rArray ); 
  ...
  for(unsigned i=0; i<ecalIs.size(); i++) {
    ...
    unsigned tmpi = reconstructClusterNew( *clusterref, 
                                        particleEnergy , rArray);
...
unsigned 
PFAlgo::reconstructClusterNew(const reco::PFCluster& cluster,
                           double particleEnergy, 
			   TClonesArray &rArray,
                           bool useDirection, 
			   double particleX,
  ...
  // depth info
  setHcalDepthInfoNew(pfCandidates_->back(), cluster, rArray);
...
void
PFAlgo::setHcalDepthInfoNew(reco::PFCandidate & cand, const reco::PFCluster& cluster, TClonesArray & rArray) const {
    ...
    pPF->e        = cand.energy(); 
    pPF->q        = cand.charge();
    pPF->pfType   = cand.particleId(); 
    pPF->ecalE    = cand.ecalEnergy();
    pPF->hcalE    = cand.hcalEnergy(); 
   
    std::cout << cand.pt() << " -- " << cand.eta() << " -- " << cand.phi() << " -- " << cand.ecalEnergy() << " -- " << cand.hcalEnergy() << std::endl; 
    ...
    if (sum > 0) {
         for (unsigned int i = 0; i < depthFractions.size(); ++i) {
           pPF->depthFrac[i] = energyPerDepth[i]/sum;
 	   pPF->depthE[i]    = energyPerDepth[i];
        }
    } else {
        std::fill(depthFractions.begin(), depthFractions.end(), 0.f);
    }
